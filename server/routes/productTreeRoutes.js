...
router.get("/", async (req, res) => {
  try {
    const productTrees = await ProductTree.find({})
      .populate("components.inventoryItem", "name itemCode")
      .populate("product");
...
router.post("/", async (req, res) => {
  const { name, description, components, product } = req.body;
  try {
    const newProductTree = new ProductTree({
      name,
      description,
      components,
      product,
    ...
    if (error.code === 11000) {
      return res
        .status(400)
        .json({ msg: "Bu isimde bir ürün ağacı zaten mevcut." });
    }
...
router.get("/:id", async (req, res) => {
  try {
    const tree = await ProductTree.findById(req.params.id)
      .populate("components.inventoryItem")
      .populate("product");
...
router.put("/:id", async (req, res) => {
  try {
    ...
    const updatedTree = await ProductTree.findByIdAndUpdate(
      req.params.id,
      { name, description, components, product },
      { new: true }
    );
    if (!updatedTree)
      return res.status(404).json({ msg: "Ürün ağacı bulunamadı." });

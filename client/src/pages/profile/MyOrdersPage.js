...
const getStatusDetails = (status) => {
  const statusMap = {
    'Onay Bekliyor': { className: 'status-onay-bekliyor', icon: 'fas fa-hourglass-half' },
    'Hazırlanıyor': { className: 'status-hazırlanıyor', icon: 'fas fa-box-open' },
    'Kargoya Verildi': { className: 'status-kargoya-verildi', icon: 'fas fa-truck' },
    'Teslim Edildi': { className: 'status-teslim-edildi', icon: 'fas fa-check-circle' },
    'İptal Edildi': { className: 'status-iptal-edildi', icon: 'fas fa-times-circle' },
...
function MyOrdersPage() {
  ...
  const filterOrders = () => {
    switch (activeFilter) {
      case 'Devam Edenler':
        ...
        return orders.filter(order => ['Onay Bekliyor', 'Hazırlanıyor', 'Kargoya Verildi'].includes(order.status));
  ...
  const handleCancelBackorder = async (orderId, productId) => {
    try {
      const res = await fetch(`/api/orders/${orderId}/backordered/${productId}/cancel`, {
        method: 'PUT',
    ...
    <div className="my-orders-page">
      ...
      <div className="order-filters">
        {filterButtons.map(filter => (
          <button 
            key={filter}
            className={`filter-btn filter-btn-${filterButtonClasses[filter]} ${activeFilter === filter ? 'active' : ''}`}
              ...
              <div key={order._id} className="order-card">
                ...
                <div className="order-body">
                  ...
                  {order.backorderedItems && order.backorderedItems.length > 0 && (
                    <>
                      <hr />
                      <div className="backorder-header">Eksikteki Ürünler</div>
                      {order.backorderedItems.map(item => (
                        <div key={item.product} className="order-item backordered-item">
                          <span className="badge-backordered">Eksikte</span>
                          {item.qty} x {item.name}
                          <button className="cancel-backorder-btn" onClick={() => handleCancelBackorder(order._id, item.product)}>
                            İptal Et
                          </button>
                        </div>
                      ))}

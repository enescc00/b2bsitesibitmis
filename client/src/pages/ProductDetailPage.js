...
function ProductDetailPage() {
    ...
    if (loading) return <div className="loading-container">Ürün Yükleniyor...</div>;
    if (error) return <div className="error-container">Hata: {error}</div>;
    if (!product) return <div>Ürün bulunamadı.</div>;

    const imageArray = product.images && product.images.length > 0 ? product.images : [];
        ...
        <div className="product-detail-page">
            <div className="product-detail-main">
                ...
                <div className="product-info">
                    ...
                    <div className="product-meta">
                        <span>SKU: <strong>{product.sku || 'Belirtilmemiş'}</strong></span>
                        <span>Garanti: <strong>{product.warrantyPeriod || 'N/A'}</strong></span>
                    </div>
                    <div className="product-price-section">
                        <span className="current-price">{(product.salePrice || 0).toFixed(2)} ₺</span>
                    </div>
                    <div className="product-actions">
                        ...
                        <button className="add-to-cart-button" onClick={handleAddToCart} disabled={product.stock < 1}>
            ...
            <div className="product-detailed-info">
                <div className="tab-buttons">
                    <button className={activeTab === 'info' ? 'active' : ''} onClick={() => setActiveTab('info')}>Ürün Bilgileri</button>
                    <button className={activeTab === 'reviews' ? 'active' : ''} onClick={() => setActiveTab('reviews')}>Ürün Yorumları ({product.numReviews})</button>
                </div>
                    ...
                    {activeTab === 'info' && (
                        <div className="info-tab">
                            ...
                            {product.specifications && product.specifications.length > 0 && (
                                <><h3>Teknik Özellikler</h3><table className="specs-table"><tbody>{product.specifications.map((spec, index) => (<tr key={index}><td>{spec.key}</td><td>{spec.value}</td></tr>))}</tbody></table></>
                            )}
                            {product.boxContents && product.boxContents.length > 0 && (
                                <><h3>Kutu İçeriği</h3><table className="specs-table"><tbody>{product.boxContents.map((content, index) => (<tr key={index}><td>{content.quantity} Adet</td><td>{content.item}</td></tr>))}</tbody></table></>
                            )}
                        </div>
                    )}
                    {activeTab === 'reviews' && (
                        <div className="reviews-tab">
                            ...
                            <div className="reviews-list">
                                {product.reviews.map(review => (
                                    <div key={review._id} className="review-item">
                                        ...
                                        <p className="review-date">{new Date(review.createdAt).toLocaleDateString('tr-TR')}</p>
                                        <p>{review.comment}</p>
                                    </div>
                                ))}
                            </div>
                            <div className="review-form-container">
                                ...
                                {user ? (
                                    ...
                                    <p>Yorum yapmak için lütfen <a href="/login">giriş yapın</a>.</p>
                                )}
                            </div>
                        </div>
                    )}

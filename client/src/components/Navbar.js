    ...
    const fetchAndProcessCategories = async () => {
        try {
            ...
            const processedMenu = topLevelCategories.map(parent => ({
                ...parent,
                children: subCategories.filter(child => String(child.parent) === String(parent._id))
            }));
    ...
    <header className="header-container">
        <nav className="navbar">
          <div className="navbar-top">
            ...
            <form className="search-container" onSubmit={searchHandler} ref={searchContainerRef}>
                <input 
                    type="text" 
                ...
                {user ? (
                  <div className="account-menu">
                    ...
                    <div className="dropdown-content">
                      ...
                      <Link to="/profile/wishlist" className="dropdown-link">Favorilerim</Link>
                      <Link to="/profile/quotes" className="dropdown-link">Tekliflerim</Link>
                      <Link to="/new-quote" className="dropdown-link">Yeni Teklif</Link>
                      
                      {/* === YENİ EKLENEN KISIM: Satış Temsilcisi Portalı Linki === */}
                      {user.role === 'supplier' && (
                        <Link to="/supplier/dashboard" className="dropdown-link supplier-link">Tedarikçi Portalı</Link>
                      )}
                      {user.role === 'sales_rep' && (
                        <Link to="/portal/dashboard" className="dropdown-link sales-rep-link">Pazarlamacı Portalı</Link>
                      )}
                      
                      {user.role === 'admin' && (
                        <Link to="/admin/dashboard" className="dropdown-link admin-link">Admin Paneli</Link>
                      )}
          ...
          <div className="navbar-bottom">
              <Link to="/products" className="nav-link main-category">TÜM ÜRÜNLER</Link>
              {menuItems.map(category => (
                  <div key={category._id} className="mega-menu-item">
                      <Link to={`/category/${category._id}`} className="nav-link main-category">
                          {category.name}
                      </Link>
                      {category.children.length > 0 && (
                          <div className="mega-menu-content">
                              {category.children.map(child => (
                                  <Link key={child._id} to={`/category/${child._id}`} className="sub-category-link">
                                      {child.name}
                                  </Link>
                              ))}
                          </div>
